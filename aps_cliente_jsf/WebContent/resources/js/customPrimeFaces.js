/**
 * Customiza a seleção de colunas a serem apresentadas no componente
 * columnToggler. As colunas com a class not-show-at-start não serão exibidas
 * por default.
 */
PrimeFaces.widget.ColumnToggler.prototype.render = function() {
	...
	for (var a = 0; a < this.columns.length; a++) {
		...
		$(
				'<li class="ui-columntoggler-item"><div class="ui-chkbox ui-widget"><div id="cb'
						+ id /*
								 * creating id for each checkbox for accessing
								 * later
								 */
						+ '" class="ui-chkbox-box ui-widget ui-corner-all ui-state-default ui-state-active"><span class="ui-chkbox-icon ui-icon ui-icon-check"></span></div></div><label>'
						+ b.children(".ui-column-title").text()
						+ "</label></li>").data("column", b.attr("id"))
				.appendTo(this.itemContainer);

		// access clumns using class reference(not-show-at-start) created in jsf
		// page
